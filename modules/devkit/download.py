"""``devkit download`` — clobber ``_shared/`` from the installed package, then regenerate.

Steps:
1. Refuse to run if ``_shared/`` has uncommitted local modifications (caller should route through
   :mod:`modules.devkit.sync`, which asks the user what to do).
2. ``rm -rf _shared/`` and copy the packaged ``content/`` tree into it verbatim.
3. :func:`modules.devkit.render.render_repo` to rewrite every provider view.
4. :mod:`modules.devkit.check` for the drift gate.
"""
...
def download(repo_root: Path, *, force: bool = False) -> RenderResult:  # noqa: ARG001
    ...
    ``rm -rf`` + copy from :func:`modules.devkit.content.packaged_content_root`, then
    :func:`modules.devkit.render.render_repo`.
    """
