...
| "show the whole backlog", "what's open anywhere \| across the family", "any bugs open in any repo" | `backlog.list --all` (grouped by repo, empty repos shown as one `none` line; `--scope ai\|dev_prd` to narrow, same `--type` / `--label` / `--mine` filters apply per repo) |
