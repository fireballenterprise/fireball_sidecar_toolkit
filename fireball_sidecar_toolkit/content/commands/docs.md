---
name: docs
description: Audit the repo for doc/AI-config drift against recent changes and fix anything stale (READMEs, ai/shared/ + ai/local/, AGENTS.md, CLAUDE.md, and the generated provider dirs).
...
3. **`ai/shared/instructions/*.md` + `ai/local/instructions/*.md`** — the source of truth for all
   AI rules; look for now-stale references to removed/renamed modules, commands, tasks, or config
   keys. (Edit `ai/shared/` via `fireball_sidecar_toolkit`'s `content/`, not in place.)
4. **`AGENTS.md` / `CLAUDE.md`** — generated pointer files; regenerated by `download`, never
   hand-edited.
5. **Generated AI-provider files** — `.github/prompts/`, `.github/instructions/`, `.claude/`,
   ...
   `ai/local/`) and re-run `invoke sidecar.toolkit.download`.
