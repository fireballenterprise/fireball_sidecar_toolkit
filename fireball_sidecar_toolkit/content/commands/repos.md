...
First, read `.ai/toolkit/instructions/repos.md` in full — it's the source of truth for
...
- If the user wants a change applied to the related repos (e.g. "apply this to the related repos" /
  ...
  `.ai/toolkit/instructions/repos.md` against each repo in scope.
...
`.ai/toolkit/instructions/repos.md` and act on it — even if they didn't explicitly run `/repos`.
