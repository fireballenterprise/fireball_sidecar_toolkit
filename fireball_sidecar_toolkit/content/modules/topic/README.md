...
- `update_list.py` — get/add/check entries in `topics/topics_list.yml` (`topics:` index + the
  optional `topic_meta:` map of `path -> {description, instructions}`)
...
- `new.py` — create a new topic directory at `topics/<path>` and initialize it via `init.scaffold()`
  (`topic.new` / `/topic new`)
- `switch.py` — auto-save any chat active in the outgoing topic, validate the target topic exists
  (fuzzy-match suggestions on typos), update `active_topic.yml`, surface the new topic's `AGENTS.md`
  and any resumable active chat (`topic.switch` / `/topic switch`, or bare `/topic <path>`)
- `list.py` — show the active topic, or every topic (`--all`) with the active one starred
...
- `route.py` — routes `/topic <subcommand> ...` arguments to the module above matching that
  subcommand (`init`, `list`, `new`, `switch`, `update`), defaulting to `switch` when the first
  token isn't a known subcommand (the bare `/topic <path>` shorthand)
- `README.md` — this file
...
- No nested-topic tooling beyond what's needed today — topics are flat, though the directory
  layout and `templates.py` support arbitrary nesting depth if that's needed later
