...
judgment — see `logic.instructions.md`), invoke tasks are deterministic: no judgment calls, no
...
| Task | Description |
|---|---|
| `download` | clobber `_shared/` from the package, regenerate every provider file |
| `check` | read-only drift gate (runs inside `invoke test`) |
| `sync` | inspect `_shared/` for local edits → offer upload → download |
| `upload` | open a PR against the toolkit with local `_shared/` edits |

Never hand-edit a generated provider file. See `ai_commands.instructions.md`.
