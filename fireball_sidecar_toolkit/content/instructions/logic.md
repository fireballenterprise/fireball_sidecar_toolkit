...
modules/*/            All business logic. Reusable, testable. The only place anything
        ...
        └── tasks/*.py (invoke)  Thin CLI wrapper too, reached via `invoke <task>`. Used for
                                  ...
                                  — see `.ai/shared/instructions/tasks.md`.
        ↑
...
`content/commands/`, mirrored into each consuming repo as `.ai/shared/commands/`). Those bodies are
the single source of truth every AI tool reads for command *behavior* (see
`.ai/shared/instructions/ai_commands.md` for authoring them). If a decision isn't written down in a
command body or a standing rule under `.ai/shared/instructions/` (or `.ai/local/instructions/`), an
...
`.ai/shared/commands/<slug>.md` file and running the same
`uv run --no-sync python -m modules.*.route "..."` / `invoke` commands — with zero AI involved?* If
...
rules live under `.ai/shared/instructions/`, no business logic and no decision logic is tied to any
...
`allowed-tools`, `name`, …) plus a single line back to `.ai/shared/<kind>/<slug>.md` (or
`.ai/local/…`). No canonical body text is inlined anywhere. This includes Copilot's
...
`AGENTS.md`, `CLAUDE.md`). Edit `.ai/shared/` (via the toolkit) or `.ai/local/` and run
...
| **Claude Code** | `CLAUDE.md` → `AGENTS.md` → `.ai/shared|local/instructions/`; `.claude/commands/` + `.claude/skills/` (stubs → `.ai/`) | |
| **Fireball Sidecar** | `AGENTS.md` + `.sidecar/` stubs → `.ai/shared|local/` | |
...
- `.ai/shared/instructions/tasks.md` — invoke task runner (plain CLI automation, no AI)
- `.ai/shared/instructions/modules.md` — Python module architecture and layout conventions
- `.ai/shared/instructions/tests.md` — testing requirements and workflow

Repo-specific instructions live in the consuming repo's `.ai/local/instructions/` — the generated
