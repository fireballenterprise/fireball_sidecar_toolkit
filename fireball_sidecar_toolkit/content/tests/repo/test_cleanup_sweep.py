...
def test_find_orphan_dirs_only_flags_untracked_trees(repo):
    assert {p.name for p in cleanup._find_orphan_dirs(repo)} == {"orphan"}
...
def test_sweep_removes_trash_keeps_tracked_and_topics(repo, monkeypatch):
