...
# Clobbered from the toolkit — fix the markdown in `content/` upstream, never here.
_CLOBBERED_PREFIXES = ((".ai", "toolkit"), ("modules", "toolkit"), ("tasks", "toolkit"), ("tests", "toolkit"))
...
def _iter_markdown(root: Path, exclude: tuple[str, ...]):
    ...
    for path in sorted(root.rglob("*.md")):
        ...
        if not _SKIP_DIRS.isdisjoint(parts):
            continue
        if parts[:2] in _CLOBBERED_PREFIXES:  # clobbered from the toolkit — fix content/ upstream
            continue
