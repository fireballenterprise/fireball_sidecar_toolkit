...
def download(context):  # noqa: ARG001
    """Clobber _shared/ from the installed toolkit, then regenerate every provider view."""
...
def check(context):  # noqa: ARG001
    """Read-only drift gate — fail if any generated provider file (or _shared/) is stale."""
    _check(Path.cwd())
    print("No drift.")
...
def sync(context, force=False):  # noqa: ARG001
    """Inspect _shared/ for local edits; if clean (or force), clobber + regenerate."""
