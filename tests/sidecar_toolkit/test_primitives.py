...
def test_download_clobbers_shared_and_renders(repo: Path):
    ...
    assert (repo / "_shared" / "commands").is_dir()
    assert (repo / "_shared" / "instructions").is_dir()
...
def test_check_raises_when_shared_is_missing(repo: Path):
    with pytest.raises(DriftError, match="_shared"):
        check(repo)
...
def test_download_refuses_dirty_shared(repo: Path):
    ...
    _git(repo, "add", "-A")
    _git(repo, "commit", "-qm", "generated")
    (repo / "_shared" / "commands" / "fix.md").write_text("tampered\n")
