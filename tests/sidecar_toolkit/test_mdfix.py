...
def test_generated_files_are_skipped(tmp_path):
    gen = tmp_path / "AGENTS.md"
    gen.write_text("<!-- DO NOT EDIT. Generated by fireball_sidecar_toolkit … -->\n\n# Index\n\ntext\n")
    assert fix_tree(tmp_path, write=True) == []
    check_tree(tmp_path)  # no raise despite the blank-after-header in the generated file


def test_git_ignored_paths_are_skipped(tmp_path):
    subprocess.run(["git", "-C", str(tmp_path), "init", "-q"], check=True)
    (tmp_path / ".gitignore").write_text("tmp/\n")
    (tmp_path / "tmp").mkdir()
    (tmp_path / "tmp" / "scratch.md").write_text("## S\n\nignored scratch, do not touch\n")
    (tmp_path / "kept.md").write_text("## S\n\nfix me\n")

    changed = fix_tree(tmp_path, write=True)
    assert [p.name for p in changed] == ["kept.md"]
    assert (tmp_path / "tmp" / "scratch.md").read_text() == "## S\n\nignored scratch, do not touch\n"
    check_tree(tmp_path)  # passes despite the untouched git-ignored file
